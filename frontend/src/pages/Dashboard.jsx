...
export default function Dashboard() {
  ...
  const loadStats = async () => {
    try {
      const [familyRes, categoryRes, unknownRes, statusRes] =
      ...
      setStats({
        family_count:
          familyRes.status === "fulfilled" ? familyRes.value.data.length : 0,
        category_count:
          categoryRes.status === "fulfilled" ? categoryRes.value.data.length : 0,
        unknown_count:
          unknownRes.status === "fulfilled" ? unknownRes.value.data.length : 0,
        surveillance_running:
          statusRes.status === "fulfilled"
            ? !!statusRes.value.data.is_running
            : false,
      });
    } catch (error) {
